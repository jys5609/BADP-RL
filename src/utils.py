...
def load_model(file_name):
    ...
    with open(file_name, 'rb') as f:
        return torch.load(f, map_location=lambda storage, loc: storage)   # modified by jmlee. load in CPU
...
class CondorContextGenerator(object):
    """Dialogue context generator. Generates contexes from the file."""
    def __init__(self, context_file, num_condor_nodes, node_idx):
        self.ctxs = []
        with open(context_file, 'r') as f:
            ...
            for line in f:
                ...
                if len(ctx_pair) == 2:
                    ...
                    ctx_pair = []
        N = len(self.ctxs)
        per_node_N = int(N / float(num_condor_nodes) + 0.5)
        from_idx = per_node_N * node_idx
        end_idx = np.min([per_node_N * node_idx + per_node_N, len(self.ctxs)])
        self.ctxs = self.ctxs[from_idx:end_idx]
        print('CondorContextGenerator: %d ~ %d / cnt=%d / N=%d' % (from_idx, end_idx, end_idx + 1, N))

        assert node_idx >= 0 and node_idx < N
