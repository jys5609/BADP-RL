...
class WordCorpus(object):
    def __init__(self, domain, path, freq_cutoff=2, train='train.txt',
        ...
        self.word_dict, self.item_dict, self.context_dict, self.item_dict_old, self.count_dict = create_dicts_from_file(
            domain,
            #os.path.join(path, train), original code
            os.path.join('data/negotiate', train),
    ...
    def _split_into_batches(self, dataset, bsz, shuffle=True):
        ...
        for i in range(0, len(dataset), bsz):
            ...
            mask = torch.zeros_like(inpt)
            for col_idx in range(inpt.size(1)):
                mask_val = 0.
                for row_idx in range(inpt.size(0)):
                    if inpt[row_idx][col_idx].item() == 5: # YOU:
                        mask_val = 1
                    elif inpt[row_idx][col_idx].item() == 6: # THEM:
                        mask_val = 0

                    mask[row_idx][col_idx] = mask_val

            mask = mask.long()
            mask = mask.view(-1)

            batches.append((ctx, inpt, tgt, sel_tgt, mask))
